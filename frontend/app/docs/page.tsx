...
export const metadata: Metadata = {
  ...
  description: "Reference for Shadowbot's routing, geocoding, and track-intelligence tools.",
};

type Confidence = "DETERMINISTIC" | "HEURISTIC" | "STATISTICAL" | "RAW DATA" | "EXTERNAL LOOKUP";
    ...
    tools: [
      {
        ...
        icon: Route,
        method: "Dijkstra shortest path (NetworkX) over a cached OSM road graph (OSMnx)",
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <header className="sticky top-0 z-10 flex items-center gap-2 border-b border-border bg-background/90 px-4 py-3 backdrop-blur">
        <Image src="/logo.png" alt="Shadowbot" width={20} height={20} className="size-5 rounded-full" />
        ...
        <div className="relative mx-auto flex max-w-5xl flex-col items-center gap-8 px-6 py-16 text-center sm:py-24 md:flex-row md:items-center md:gap-12 md:text-left">
          ...
          <div className="flex flex-col items-center gap-4 md:items-start">
            ...
            <p className="max-w-md text-sm leading-relaxed text-muted-foreground sm:text-base">
              An offline geolocation console: plan routes, triage points of interest, and mine GPS
              track history for patterns — all over locally cached map data, no live network
              dependency required.
            </p>
          </div>
        </div>
      ...
      <section className="mx-auto max-w-5xl px-6 py-14">
        ...
        <div className="mt-6 grid gap-px overflow-hidden border border-border bg-border sm:grid-cols-3">
          {[
            ...
            {
              name: "Routes",
              detail: "Plan, reroute, and compare routes directly, with avoidance constraints and arrival estimates.",
            },
            {
              name: "Geodata",
              detail: "Browse uploaded GPS tracks and their derived frequented locations on the map.",
            },
          ].map((mod) => (
            <div key={mod.name} className="flex flex-col gap-2 bg-background p-5">
              <span className="font-mono text-[11px] tracking-[0.15em] text-foreground uppercase">
                {mod.name}
              </span>
              <p className="text-sm leading-relaxed text-muted-foreground">{mod.detail}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto max-w-5xl px-6 py-14">
        ...
        <p className="mt-3 max-w-2xl text-sm leading-relaxed text-muted-foreground">
          Every action the agent can take, and the algorithm behind it. Each entry is tagged by how
          much to trust its output at face value: deterministic and statistical methods compute an
          exact answer from real data, heuristic methods estimate from a rule of thumb.
        </p>
                ...
                {group.tools.map((tool) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <code className="font-mono text-[11px] text-muted-foreground">{tool.functions}</code>
